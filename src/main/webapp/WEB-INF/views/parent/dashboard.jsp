                ...
                <div class="student-card ${not empty student.registration ? 'registered' : 'unregistered'}">
                    ...
                    <div class="card-body">
                        <c:if test="${not empty student.registration}">
                            ...
                            <div class="trip-info">
                                <h4><i class="fas fa-arrow-up" style="color: #007bff;"></i> Thông tin chuyến đón hôm nay</h4>
                                <c:choose>
                                    <c:when test="${not empty student.registration.pickupTrip}">
                                        <div class="info-row">
                                            <i class="fas fa-bus"></i> 
                                            <strong>Xe:</strong> ${student.registration.pickupTrip.bus.licensePlate}
                                        </div>
                                        <div class="info-row">
                                            <i class="fas fa-user-tie"></i> 
                                            <strong>Lái xe:</strong> ${student.registration.pickupTrip.driver.fullName}
                                        </div>
                                        <div class="info-row">
                                            <i class="fas fa-spinner"></i>
                                            <strong>Trạng thái chuyến:</strong> ${student.registration.pickupTrip.status}
                                        </div>
                                        <div class="info-row">
                                            <i class="fas fa-check-circle"></i> 
                                            <strong>Trạng thái của bé:</strong>
                                            <c:choose>
                                                <c:when test="${not empty student.registration.pickupAttendance}">
                                                    <c:set var="status" value="${student.registration.pickupAttendance.status}"/>
                                                    <span class="status-badge
                                                         ...
                                                         ${status == 'boarded' ? 'Đã lên xe' :
                                                          status == 'completed' ? 'Đã đến trường' :
                                                          status == 'notified_absence' ? 'Vắng' : 'Chưa đến'}
                                                    </span>
                                                    <c:if test="${not empty student.registration.pickupAttendance.checkInTime}">
                                                        (Lúc <fmt:formatDate value="${student.registration.pickupAttendance.checkInTime}" pattern="HH:mm" />)
                                                    </c:if>
                                                </c:when>
                                                <c:otherwise>
                                    ...
                                    <c:otherwise>
                                        <p>Chưa có thông tin chuyến đón cho hôm nay.</p>
                                    </c:otherwise>
                                </c:choose>
                            </div>

                            <div class="trip-info">
                                <h4><i class="fas fa-arrow-down" style="color: #28a745;"></i> Thông tin chuyến trả hôm nay</h4>
                                <c:choose>
                                    <c:when test="${not empty student.registration.dropoffTrip}">
                                        <div class="info-row">
                                            <i class="fas fa-bus"></i> 
                                            <strong>Xe:</strong> ${student.registration.dropoffTrip.bus.licensePlate}
                                        </div>
                                        <div class="info-row">
                                            <i class="fas fa-user-tie"></i> 
                                            <strong>Lái xe:</strong> ${student.registration.dropoffTrip.driver.fullName}
                                        </div>
                                        <div class="info-row">
                                            <i class="fas fa-spinner"></i>
                                            <strong>Trạng thái chuyến:</strong> ${student.registration.dropoffTrip.status}
                                        </div>
                                        <div class="info-row">
                                            <i class="fas fa-check-circle"></i> 
                                            <strong>Trạng thái của bé:</strong>
                                            <c:choose>
                                                <c:when test="${not empty student.registration.dropoffAttendance}">
                                                    <c:set var="status" value="${student.registration.dropoffAttendance.status}"/>
                                                    <span class="status-badge
                                                          ${status == 'boarded' ? 'status-checked-out' : ''}
                                                          ${status == 'completed' ? 'status-checked-out' : ''}
                                                          ${status == 'notified_absence' ? 'status-absent' : ''}">
                                                        ${status == 'boarded' ? 'Đã lên xe' :
                                                          status == 'completed' ? 'Đã xuống điểm trả' :
                                                          status == 'notified_absence' ? 'Vắng' : 'Chưa đến'}
                                                    </span>
                                                    <c:if test="${not empty student.registration.dropoffAttendance.checkOutTime}">
                                                        (Lúc <fmt:formatDate value="${student.registration.dropoffAttendance.checkOutTime}" pattern="HH:mm" />)
                                                    </c:if>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="status-badge status-pending">Chưa điểm danh</span>
                                    ...
                                    <c:otherwise>
                                        <p>Chưa có thông tin chuyến trả cho hôm nay.</p>
                                    </c:otherwise>
                                </c:choose>
                            </div>
